...
export const TerrainElevationProfile: React.FC<TerrainElevationProfileProps> = ({
  ...
  return (
    <div className={`w-full min-w-0 ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-[#0c0e12] border-slate-800'} rounded-lg p-4 font-mono text-xs space-y-3`}>
      ...
      <div className="relative w-full min-w-0 overflow-hidden select-none">
        <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} preserveAspectRatio="none" className="block w-full h-auto aspect-[5.45/1]">
          <defs>
            ...
            {[0, 0.25, 0.5, 0.75, 1.0].map((factor) => {
              ...
              return (
                <g key={`grid-y-${factor}`}>
                  ...
                  <text x={paddingLeft - 6} y={y + 3} fill="#64748b" fontSize="9" textAnchor="end">
                    {altVal}m
                  </text>
                </g>
              );
          ...
          {[0, 0.25, 0.5, 0.75, 1.0].map((f) => {
            ...
            return (
              <text key={`dist-${f}`} x={x} y={svgHeight - 10} fill="#64748b" fontSize="9" textAnchor="middle">
                {km} km
              </text>
            );
          })}
