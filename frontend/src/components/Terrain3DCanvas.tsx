...
export const Terrain3DCanvas: React.FC<Terrain3DCanvasProps> = ({
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      canvasRef.current.height = Math.max(360, rect.height || 460);
    };
    handleResize();
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[clamp(360px,52vw,680px)] bg-[#06080d] rounded-lg overflow-hidden select-none border border-slate-800">
