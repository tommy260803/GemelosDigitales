    ...
    <div className="space-y-6 font-sans">
      ...
      {viewMode === '3D_TOPOGRAPHIC_DEM' && (
        <div className={`flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl border ${
          theme === 'light' ? 'bg-white border-slate-200' : 'bg-[#0a0c10] border-slate-800/80'
        }`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className={`text-sm rounded-lg px-2 py-1 border focus:outline-none ${
                theme === 'light' ? 'bg-white border-slate-200 text-slate-700' : 'bg-slate-900 border-slate-700 text-amber-300'
              }`}
            >
              <option value="baseline">Línea Base (Ambulancia 2WD)</option>
              <option value="scenario_a">Escenario A (Moto-Ambulancia)</option>
              <option value="scenario_d">Escenario D (Combinado 24/7)</option>
            </select>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          {viewMode === '3D_TOPOGRAPHIC_DEM' ? (
            ...
            <div className={`relative overflow-hidden flex flex-col justify-between min-h-[480px] rounded-xl border ${
              theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-[#080a0f] border-slate-800'
            }`}>
              {/* Legend Overlay */}
              <div className={`absolute top-4 left-4 z-10 p-3 rounded-xl border shadow-lg max-w-[220px] ${
                theme === 'light' ? 'bg-white/95 backdrop-blur border-slate-200' : 'bg-slate-900/90 backdrop-blur border-slate-800'
              }`}>
              ...
              <div className="w-full h-full flex items-center justify-center pt-8 pb-2">
                <svg viewBox="0 0 800 500" className="w-full h-full max-h-[440px] drop-shadow-xl select-none">
                  {/* Grid Lines */}
              ...
              {/* Bottom Status */}
              <div className={`flex items-center justify-between text-xs px-4 py-2 border-t ${
            ...
            <div className={`mt-4 p-3 rounded-lg border ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/90 border-slate-800'}`}>
              ...
              {topographicKPI.sdTravelTimeDiscrepancyPercent > 20 && (
                <div className={`mt-3 p-2 rounded text-xs flex items-start gap-2 ${
                  theme === 'light' ? 'bg-amber-50 text-amber-700' : 'bg-amber-950/30 text-amber-200/90'
                }`}>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-xs font-semibold truncate max-w-[140px] ${theme === 'light' ? 'text-slate-700' : 'text-slate-200'}`}>
                        {fac.name}
                      </span>
